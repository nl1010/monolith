...
void loop() {
  ...
  Wire.requestFrom(SLAVE1_ID, 1); // request 1 byte from the target slave. 
  while (Wire.available()>0) {    // keep listenning in each loop, until detected receive byte from the target slave do something futhur 
